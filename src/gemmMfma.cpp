...
static DataMem<float> * g_DataA, *g_DataB, *g_DataC, *g_DataD, *g_DataRef;
static DataMem<float> * g_hfDataA, *g_hfDataB, *g_hfDataC, *g_hfDataD, *g_hfDataRef;
static DataMem<float> * g_bfDataA, *g_bfDataB, *g_bfDataC, *g_bfDataD, *g_bfDataRef;
...
void GemmMfmaProblem::initDataMem()
{
	...
	LOG("stride: A,B,C,D = %d, %d, %d.", StrideA0, StrideB0, StrideC0, StrideD0);
	...
	g_DataRef = newRealData<float>("matrix-ref", -55.55f*(int)g_dataInit, StrideD0, N*Batch);
	...
	if (g_DataType == 2)
	{
		...
		g_hfDataRef = newRealData<float>("matrix-ref", -55.55f*(int)g_dataInit, StrideD0, N*Batch);
	...
	if (g_DataType == 3)
	{
		...
		g_bfDataRef = newRealData<float>("matrix-ref", -55.55f*(int)g_dataInit, StrideD0, N*Batch);
